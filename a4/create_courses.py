...
with open(input_filename) as f:
    for row in csv.reader(f):
        ...
        e(cursor.execute("insert into course_offering values( %s, %s, %s, %s, %s ) on conflict (course_code, term_code) do nothing;", (code, term, name, instructor, capacity)), conn)
        [e(cursor.execute("insert into prerequisites values( %s, %s, %s ) on conflict (course_code, term_code, prereq_code) do nothing;", (code, term, pre_req)), conn) for pre_req in prerequisites]
